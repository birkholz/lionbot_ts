import { pgTable, foreignKey, bigint, varchar, integer, boolean, serial, unique, jsonb, type PgTableWithColumns, timestamp } from "drizzle-orm/pg-core"


export const streams: PgTableWithColumns<any> = pgTable("streams", {
...
export const guilds: PgTableWithColumns<any> = pgTable("guilds", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const twitchStreams = pgTable("twitch_streams", {
	id: serial().primaryKey().notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	unique("twitch_streams_twitch_id_guild_id").on(table.twitchId, table.guildId),
]);
