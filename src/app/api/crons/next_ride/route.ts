...
import { isCronTimeValid, sendDiscordRequest } from "@lib/utils"
...
export async function GET() {
  ...
  try {
    ...
    const dbRides = await db
      ...
      .orderBy(asc(scenicRidesTable.last_posted_at))
    ...
    if (nextRide) {
      ...
      const jsonBody = {
        content:
          ...
          `Published: ${nextRide.published_date}\n` +
          "-# Lionbot always checks for new or removed scenic rides and updates the schedule accordingly. If there are any issues, defer to #peloton",
        allowed_mentions: {
          parse: ["roles"],
        },
      }
      ...
      // Update the last_posted_at timestamp for this ride
      await db
        .update(scenicRidesTable)
        .set({ last_posted_at: sql`CURRENT_DATE` })
        .where(sql`id = ${nextRide.id}`)
    }
